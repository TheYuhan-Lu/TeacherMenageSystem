...
{
    os << " " << a.t_id << " " << a.t_name << " " << a.t_unit << " " << a.t_number
        << " " << a.t_basic_salary << " " << a.t_bonus << " " << a.t_tax << " " << a.t_fund << " " << a.t_sum_exact << endl;
    return os;
}
...
bool search(string a, string b) //模糊查找
{
    ...
    for (int i = 0; i < int(a.size());++i)
    {
        ...
        {
            int j = i + 1;
            for (int x = 1; x < int(b.size()); ++j, ++x)
            {
                if (a.at(j) == b.at(x)) d = 1;
                else
                {
                    d = 0; break;
                }
                if ((x == int(b.size()) - 1) && (d == 1))return 1;
            }
        }
    }
    return 0;
